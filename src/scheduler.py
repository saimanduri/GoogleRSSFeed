    ...
    def add_collection_job(self, job_function: Callable[[], None]):
        ...
        for time_str in self.times:
            try:
                ...
                logger.error(f"Error scheduling job at {time_str}: {e}")
...
def initialize_scheduler(config_manager) -> Optional[FeedScheduler]:
    """
    ...
    Returns:
        Initialized scheduler or None if failed
    """
    try:
        schedule_times = config_manager.get_config_value("schedule.times", ["05:00", "14:00"])
        timezone = config_manager.get_config_value("schedule.timezone", "Asia/Kolkata")
        
        if not schedule_times:
            logger.error("No schedule times configured")
            return None
        
        scheduler = FeedScheduler(schedule_times, timezone)
        ...
        def collection_job():
            try:
                collector = RSSCollector(config_manager.settings_path)
                return collector.run_collection()
            except Exception as e:
                logger.error(f"Error in collection job: {e}")
                return None
        
        scheduler.add_collection_job(collection_job)
